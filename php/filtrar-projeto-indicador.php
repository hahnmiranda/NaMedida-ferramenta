	...
	if (mysqli_num_rows($resultado) > 0):
		...
		if (mysqli_num_rows($resultado) > 0):
			...
			while($indicadores_dados = mysqli_fetch_array($resultado)):
				$ids_medidas[] = $indicadores_dados['idIndicador'];
			endwhile;
...
<div class="row">
	...
	<?php if(count($ids_medidas) > 0): ?>
		<h3 class="light"> Esta é sua lista de indicador(es) filtrada pelo projeto</h3>
		<h5 class="light"></h5>
		<table class="striped responsive-table">
			...
			<tbody>
				...
				?>
				<h3 class="light"> Nenhum indicador encontrado para este projeto</h3>
				<?php
				endif;
				?>
			</tbody>
